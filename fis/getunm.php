<?php 
	...
	// $x = file_get_contents("http://www.fis.unm.si/si/za-studente/knjiznica/e-knjiznica/diplomske-naloge-vs/");
	// $x = file_get_contents("http://www.fis.unm.si/si/za-studente/knjiznica/e-knjiznica/diplomske-naloge-uni/");
	$x = file_get_contents("http://www.fis.unm.si/si/za-studente/knjiznica/e-knjiznica/magistrske-naloge/");
	...
	foreach($ar[1] as $row) {
		...
		$avtor = trim(strip_tags(trim($ar2[1][0])." ".trim($ar2[1][1])));
		$naslov = trim(strip_tags($ar2[1][2]));
		$datum = trim(strip_tags($ar2[1][3]));
		$url = $ar2[1][4];
		$dat = explode(".",$datum);
		$year = $dat[2];
